...
def my_train_off_policy_agent(bvh_folder,agent, num_episodes, replay_buffer, minimal_size, batch_size,writer,explore_cnt=100,des=""):
    return_list = []
    # 加载num条原始数据
    data_root = [os.path.join(bvh_folder, "complete"), os.path.join(bvh_folder, "incomplete")]

    json_root=r"/home/mjy/teeth/RL/datasets"# r"E:\PROJECTS\tooth_spatial_temporal_transformer"
    mode=['test','train','val']
    remove_dict={}
    for subdir in mode:
    ...
    PROJECT_DIR = r"/home/mjy/teeth/RL/datasets"
    ...
    for i in range(num_episodes*2):
                ...
                env = gym.make('OrthoEnv',first_step=first_state,convex_hulls={},epsilon=0)
                env_list.append(env)
        ...
        for epoch in range(explore_cnt):
            ...
            for i in range(num_episodes):
                ...
                while not done:
                        ...
                        if replay_buffer.size() > minimal_size:
                            b_s, b_a, b_r, b_ns, b_d = replay_buffer.sample(batch_size)
                            transition_dict = {'states': b_s, 'actions': b_a, 'next_states': b_ns, 'rewards': b_r, 'dones': b_d}
                            critic_loss, action_lossl1,action_lossl2 = agent.update(transition_dict)
                            writer.add_scalar('critic_loss', critic_loss, iter_cnt)   # vis
                            writer.add_scalar('action_lossl1', action_lossl1, iter_cnt)   # vis
                            writer.add_scalar('action_lossl2', action_lossl2, iter_cnt)   # vis
                            iter_cnt+=1
                        if bump_cnt>150:
                            break
                temp_return_list.append(episode_return)
                writer.add_scalar('episode_return', episode_return, epoch * num_episodes + i)   # vis
                writer.add_scalar('bump_cnt',bump_cnt,epoch*num_episodes+i)  # vis
                if i%50==0:
                    tmp_mean = np.mean(temp_return_list)
                    if tmp_mean> best_return_batch:
                        best_return_batch = tmp_mean
                        save_checkpoint(r"/home/mjy/teeth/RL/checkpt/",agent,epoch,i,suffix=f"bestbatch_{agent_name}{des}.pth")
                pbar.set_postfix({'episode': '%d' % i, 'return': '%.3f' % episode_return})
            # for each epoch
            tmp_mean = np.mean(temp_return_list)
            print(f"epoch:{epoch}, mean_return:{tmp_mean}")
            writer.add_scalar('epoch_return', tmp_mean, epoch)   # vis
            if tmp_mean> best_return:
                best_return = tmp_mean
                save_checkpoint(r"/home/mjy/teeth/RL/checkpt/",agent,epoch,0,suffix=f"best_{agent_name}{des}.pth")
            ...
            for i in range(num_episodes,2*num_episodes):
                ...
                while not done:
                        ...
                        if bump_cnt>150:
                            break
                writer.add_scalar('eval_episode_return', episode_return, epoch * num_episodes + i)   # vis
                writer.add_scalar('eval_bump_cnt',bump_cnt,epoch*num_episodes+i)  # vis
                pbar.set_postfix({'eval-episode': '%d' % i, 'return': '%.3f' % episode_return})
            pbar.update(1)
    for i in range(num_episodes*2):
        env_list[i].close()
    return return_list
